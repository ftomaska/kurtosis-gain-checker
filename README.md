...
| Subtract baseline (F0) | on | Subtracts each cell's own low-percentile baseline from F before the photon-flux conversion (see below) — corrects for the digitizer's black-level offset, which otherwise inflates the flux numbers |
| Baseline pctile | 30 | Which percentile of each cell's own raw trace counts as its baseline (F0) when the above is checked |

Photon flux uses **raw F only** — no neuropil subtraction — consistent with the Wilt et al. convention that F0 includes all detected photons (signal + background *light*). That's a deliberate, separate choice from baseline subtraction: F (whether from Suite2p's `F.npy` or CNMF) is a raw per-ROI pixel **sum**, which includes the digitizer's constant black-level/dark offset — present even at zero real photons, visible as the nonzero floor on the PTC plot's Mean-ADU axis. Left in, that offset gets miscounted as detected photons, and because it's summed over the whole ROI it scales with cell size — the usual cause of implausibly large flux numbers (tens of thousands of photons/cell/s). With **Subtract baseline (F0)** checked (the default), each cell's own baseline — the `Baseline pctile`-th percentile of its raw trace, 30 by default — is subtracted before the conversion, removing most of that offset while leaving genuine background *light* (real photons, which do belong in the flux number per the Wilt convention above) alone. Uncheck it to see the original raw, un-subtracted numbers.

**Re-fit without a full re-run:** after Run Analysis has completed once, the **↻ Re-fit** button (under Fit range/ENF in the sidebar) redoes just the linear PTC fit — and, if there's a flux panel, rescales it — using the mean/variance bins already computed. It's instant (no thread, no movie re-read) because Fit range, ENF, and the baseline-subtraction settings only rework numbers that already exist (the cached bins, and the cached raw per-cell F respectively). Frame rate `fs` is also picked up live by Re-fit. Spatial bin, Edge margin, Exclude cell ROIs, and anything CNMF/motion-correction related bake into the bins themselves during the raw-pixel pass, so changing those still needs a full Run Analysis.
